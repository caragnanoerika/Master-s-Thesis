"""
Step 12 — SV-ADF results table for individual equities.

Generates a publication-ready table covering all equity tickers
(segments 1–11, same universe as script 11 for the GSADF table).
...
Output
------
    outputs/tables/svadf_equity_results.csv
    outputs/tables/svadf_equity_results.tex

Usage
-----
    python scripts/12_svadf_equity_table.py
"""
...
def main() -> None:
    ...
    csv_path = settings.TABLES_DIR / "svadf_equity_results.csv"
    df.to_csv(csv_path, index=False)
    print(f"CSV   -> {csv_path}")

    tex_path = settings.TABLES_DIR / "svadf_equity_results.tex"
