...
Usage
-----
  python scripts/07_robustness.py                    # run all checks
  python scripts/07_robustness.py --checks A2 A3 B2  # run subset
  python scripts/07_robustness.py --skip-mc           # skip A1 / A4
"""
import argparse
import sys
import time
...
def run_B2() -> pd.DataFrame:
    ...
    grid = [
        (30, 15, 60,  "B2_M30_R15_b60"),
        (60, 30, 90,  "B2_baseline"),    # settings defaults: M=60, R=30, bridge=90
        (60, 30, 60,  "B2_bridge60"),
        (90, 30, 90,  "B2_M90_R30"),
        (60, 60, 90,  "B2_M60_R60"),
    ]
    w1 = (settings.SVADF_DEFAULT_START, settings.SVADF_DEFAULT_END)
    summary = []
    for M, R, bridge, tag in grid:
        rows = []
        for tk in settings.ALL_TICKERS:
            r = load_svadf(tk, *w1)
            ...
            rows.append({
                ...
                "reject": ep is not None,
                "n_ep": int(ep is not None),
                "config": tag, "M": M, "R": R, "bridge_days": bridge,
            })
        df = pd.DataFrame(rows)
        df.to_csv(ROB_TABLES / f"rob_{tag}.csv", index=False, encoding="utf-8-sig")
        m = _metrics(rows, tag, alpha_key="reject")
