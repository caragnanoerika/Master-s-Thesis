"""
05_stationarity_table.py
Build a stationarity summary table from per-ticker JSON results.

Default mode  (no args)
  Reads cached JSON results and writes stationarity_table.csv.
...
Date-range mode  (--start / --end)
  ...
  Example:
    python scripts/05_stationarity_table.py --start 2022-11-01 --end 2026-05-01
    python scripts/05_stationarity_table.py --start 2020-01-01 --end 2022-10-31

Output: outputs/tables/stationarity_table[_<start>_<end>].csv
"""
import argparse
import json
import sys
...
if __name__ == "__main__":
    ...
    if args.start or args.end:
        ...
        tag   = f"_{start}_{end}"
        out   = settings.TABLES_DIR / f"stationarity_table{tag}.csv"
    else:
        df  = build_table_from_cache()
        out = settings.TABLES_DIR / "stationarity_table.csv"
